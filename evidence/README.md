...
Run the following commands from the project root in Windows PowerShell. Each command captures the complete `psql` output, records the exit status immediately and writes reviewable UTF-8 text. Enter passwords only at the local password prompts; never place them in a command or evidence file.
